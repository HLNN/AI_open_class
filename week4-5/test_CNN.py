...
class MNIST:
    def __init__(self):
        ...
        self.src = cv2.imread("5.jpg")
        self.p1 = ()
        self.p2 = ()

        self.src = cv2.cvtColor(self.src, cv2.COLOR_BGR2GRAY)
        if self.src.shape[0] > 1080 or self.src.shape[1] > 1920:
            factor = max(math.ceil(self.src.shape[0] / 1080), math.ceil(self.src.shape[1] / 1920))
            self.src = cv2.resize(self.src, (int(self.src.shape[0] / factor), int(self.src.shape[1] / factor)))
    ...
    def main(self):
        ...
        while True:
            ...
            cv2.waitKey(30)
            if self.predict:
                src = self.src[self.p1[0]:self.p2[0], self.p1[1]:self.p2[1]]
